...
function make_BEST_transform_func(y1, y2) {
    var data = [y1, y2]
    var mean_mu = jStat.mean(y1.concat(y2))
    var sd_mu = jStat.stdev(y1.concat(y2)) * 1000
    var sigma_low = jStat.stdev(y1.concat(y2)) / 1000
    var sigma_high = jStat.stdev(y1.concat(y2)) * 1000
...
function make_BEST_single_transform_func(y1, y2) {
    ...
    var transform = function(cube) {
        ...
        params[1] = cube[1] * (sigma_high - sigma_low) + sigma_low
        for(var i = 0; i < params.length; i++) {
            if (isNaN(cube[i]) || isNaN(params[i])) alert("nan in transform[" + i + "]: " + cube[i] + " --> " + params[i])
        }
        return params
